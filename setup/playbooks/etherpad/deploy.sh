...
sleep 15
oc new-app -f etherpad-template.yaml -p DB_USER=ether -p DB_PASS=ether -p DB_DATABASE=ether -p DB_PORT=3306 -p DB_HOST=mysql -p ADMIN_PASSWORD=secret -p DB_TYPE=mysql
