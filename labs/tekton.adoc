...
OpenShift Pipelines are based on the Tekton open source project and they provide a Kubernetes style resource based approach to the construction of pipelines. Tasks are defined as small units (typically 10 to 25 lines of YAML) and such tasks are grouped together into taskruns or pipelines for execution.

A command line exists for the execution of Tekton commands and a graphical user interface is also available which provides a simple way to run tasks and to observe running or completed tasks.

Finally a plugin exists for the Visual Studio family of editors that can ease the creation and execution of Tekton pipelines.
****

==== Further reading

Details on the syntax and use of Tekton is available in a series of blog posts starting here : https://www.openshift.com/blog/guide-to-openshift-pipelines-part-1-introducing-openshift-pipelines[https://www.openshift.com/blog/guide-to-openshift-pipelines-part-1-introducing-openshift-pipelines, window="_blank"]

Information on how to trigger pipelines from GitHub is available here :
https://www.openshift.com/blog/guide-to-openshift-pipelines-part-6-triggering-pipeline-execution-from-github[https://www.openshift.com/blog/guide-to-openshift-pipelines-part-6-triggering-pipeline-execution-from-github, window="_blank"]
...
.Tekton command line
****
A command line utility is available to manage Tekton pipelines.. The commands cover the different pipeline objects such as task, taskrun, pipeline, pipelinerun,  resources, cluster tasks, conditions etc.
...
Pipelines are used to manage the execution of a series of tasks. The example pipeline below is used to execute the Tekton tasks : task-1, task-2 and task-3. Remember that each Tekton task can have multiple steps within it.
...
Once you have executed the command switch back to the OpenShift console. Ensure that you have your pipeline project selected. Select the Developer view and you will find there is a menu option on the left hand side labelled 'Pipelines'. Select this option and the screen should look like this:

image::pipelines-2.png[Example Pipelines UI]

Once the pipeline is indicated to have finished by showing 'Succeeded' in the 'Status' column, go back to the terminal and enter the command suggested by the response to the tkn command - it will look similar to this:
...
You can click on the pipeline-run (labelled pipeline-5-run-XXXXX) and examine the logs for each of the tasks ising the web user interface.
