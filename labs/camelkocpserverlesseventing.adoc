...
Ensure you are on the Developer View (top level, select Developer)

Click on the drop down box next to 'Project: sandbox-user1' (see screen shot)

image::camekknative-1a.png[align="center"]
...
A recent change to the Knative channels mean that the Messaging Channel kind has changed.

Before continuing to to add the channels you need to edit both files, since writing, the CRD's have move from an alpha version to a released version.
Need to change alphav1 to v1.

Please edit both channel yaml files

[source]
----
cd /workspace/examples/knative

vi messages-channel.yaml
...
[source]
----
Messages channel should look like 
...
[source]
----
Words channel should look like 
...
TIP: The inmemory message channels are represented on the developer view along with the Camel K services that are consuming from them.
