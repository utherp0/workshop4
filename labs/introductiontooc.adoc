...
[source]
----
oc scale deployment/meow2 --replicas=3
oc scale deployment/nodenews --replicas=2
----
...
[source]
----
oc get deployment -o jsonpath='{.items[*].metadata.name}'
----
...
[source]
----
for depl in $(oc get deployment -o jsonpath='{.items[*].metadata.name}'); \
do oc scale deployment/$depl --replicas=1; \
done
