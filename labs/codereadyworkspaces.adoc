...
* Components - Plugins that add specific command capability such as the oc command line for interaction with OpenShift. Components also include editor plugins that can be used to configure the behaviour of the editor. 
...
Switch from the OpenShift web user interface to the CodeReady Workspace user interface using the Red Hat Applications icon shown below (the nine white square blocks), and then select CodeReady Workspaces.

image::codeready-workspaces-00.png[Accessing CodeReady Workspaces]
...
In the dropdown menu for the Devfile select 'NodeJS Express'. A DevFile is a definition of a workspace that can be shared with a development team to ensure consistency of plugins, commands and content from Git repositories.

Select the blue 'Create & Open' button which is under the text of the DevFile.
...
If you do not have a project that is suitable then please clone the project from here : git@github.com:marrober/layers.git into a Git repository under your Git ID and then use that. By doing this you will have a repository to which you can push changes from your workspace. Remove the sample project repository to leave just your repository behind. Be sure to use the ssh format of the git repository address and not the https varient.
...
Wait for the aminated graphic at the top of the page to stop and display a green circle with 'Running' next to it. Click on the name of the workspace on the bottom of the list on the left hand side as shown below which will start the workspace.
...
Back in the workspace use the project explorer window (top of the four buttons on the left hand side) to open the tree of files within the project and make a change to a source file and save it.

On the right hand side of the screen click on the cube symbol and then under User Runtimes and nodejs select "New Terminal". This shows the capabilities of the command line from within the workspace. In the terminal window cd to the layers directory and type 'git status' to see the change that you have made. Add and commit the change and then push to the github repository using the command  :

[source]
----
git add .
git commit -m "CRW change"
git push
----
...
When you are prompted to push the new component in the bottom right hand corner select "Push". This will use the OpenShift DO capability to push the configuration for the application to OpenShift where the Source-2-Image build process will be used to build the application. 
...
At the bottom of the screen a window will appear as shown below to display the progress of the creation of the application and any errors that may result.
...
At this stage the running application is not reachable so right click on the application in the OpenShift panel of CodeReady Workspaces again and select 'New URL'. For the URL name enter 'crw-app-1-url' and hit return. Select a secure URL.
...
The Devfile content could then easily be copied for sharing with the rest of your development team. The best way to do this (not performed in the workshop) is to store the contents of the Devfile in a yaml file in the root of a github project. in this manner the file may be version controlled and released to the team as required to make changes to the development environments. 
