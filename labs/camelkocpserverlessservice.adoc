...
To view the logs for the integration, click any where inside the inner squares "dotted lines"
...
After a few seconds you should get a response, and the topology view should now look similar to the picture below. The dark blue circle indicates that the service is now executing (if you switch back to toplogy view quickly you should see the Integration starting - indicated by a light blue ring)
